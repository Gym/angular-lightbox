...
type ContainerStyles = Partial<
  Record<'transition' | 'transform' | 'width' | 'height' | 'opacity', string>
>;
@Directive()
export abstract class LightboxCommonComponent {
  currentImageIndex = 0;
  showState?: ShowState;
  ...
  readonly lightboxData = inject(LIGHTBOX_DATA);
  ...
  get images(): ImageExtended[] {
    return (
      this.lightboxData.images ||
      (this.lightboxData.image ? [this.lightboxData.image] : [])
    );
  }
  ...
  get containerInitialPosition() {
    const scale =
      this.showState === 'initial-thumbnail-image' ? 1 : this.containerScale;
    const top = this.thumbnailImagePosition.top;
    const left = this.thumbnailImagePosition.left;
    return (
      'matrix(' + scale + ', 0, 0, ' + scale + ',' + left + ',' + top + ')'
    );
  }

  get containerFullscreenPosition() {
    const left =
      (document.body.clientWidth - this.virtualImageDimension.width) / 2;
  ...
  get counter() {
    return (
      this.currentImageIndex + 1 + this.counterSeparator + this.images.length
    );
  }

  constructor(public eventService: EventService) {}

  emitState(type: string, state: string) {
  ...
  setAnimationDuration() {
    ...
    this.containerStyles.transition =
      'all ' + this.animationDuration + 'ms ' + this.animationTimingFunction;
  }
